...
For the comparison in ranking with those baselines, we convert
our tool's result into a ranked list by ranking the statements in the
predicted set by the classification scores (i.e., before
...
(1) {\bf Hit-N} measures the number of times that the predicted set
from {\tool} contains {\em at least} $N$ faulty statements (i.e., the
predicted set and the correct set overlap at least $N$ statements).
...
classification scores (i.e., before deriving the final set).
