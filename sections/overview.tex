...
\vspace{3pt}
\noindent{\bf \underline{Step 2. Graph-based Feature Representation Learning.}}
...
the method FL model (\code{MethFL}) and the statement FL model
(\code{StmtFL}) to learn the co-fixing methods and co-fixing
...
The input of the prediction process includes the test cases and the
...
input of the {\em trained} \code{StmtFL} model, which predicts the
...
If one aims to predict the buggy methods, the \code{MethFL} model could be used
