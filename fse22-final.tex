...
in one model can benefit the other and vice versa.
%
Thus, we simultaneously train them with soft-sharing the models'
parameters via cross-stitch units to enable the propagation of the
impact of \code{MethFL} and \code{StmtFL} onto each other. In addition
to the new solution with dual-task learning, we explore a novel
feature for FL: the co-changed statements. We also use Graph-based
Convolution Network to integrate different types of program
...
the state-of-the-art APR tools -- DEAR and CURE. The empirical
results show that {\tool}+DEAR (the original FL in DEAR replaced by
{\tool}) and {\tool}+CURE improve relatively over the original DEAR
and Ochiai+CURE by 10.5\% and 42.9\% in terms of the number of fixed
bugs.
\end{abstract}
