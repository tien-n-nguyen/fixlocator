...
evaluation also confirmed that (Section~\ref{sensi}).
...
We conducted several experiments to evaluate {\tool} on Defects4J benchmark~\cite{defects4j}. Our empirical results show that \tool can improve the compared baselines: CNN-FL~\cite{zhang2019cnn}, DeepFL~\cite{DeepFL}, and DeepRL4FL~\cite{icse21-fl} by 16.6\%, 16.9\%, and 9.9\%, respectively, in terms of Hit-1@Set: a predicated set contains at least one buggy statement. Furthermore, we evaluated the capability of detecting multiple buggy statements in predicated sets using Hit-N@Set with N$\ge$2. Our {\tool} can locate more buggy statements than CNN-FL, DeepFL, and DeepRL4FL by 33.6\%, 40.3\%, and 26.5\% in
terms of Hit-2@Set, 43.9\%, 46.4\%, and 28.1\% in terms of Hit-3@Set,
100\%, 155.6\%, and 64.5\% in terms of Hit-4@Set, respectively. {\tool} can also reduce the EXAM scores of CNN-FL, DeepFL, and DeepRL4FL by 22.2\%, 30\%, and 22.2\%, respectively. The EXAM score is a well-known indicator of evaluating the searching efforts for locating buggy statements. The lower EXAM score of {\tool} show that {\tool} can save more human efforts in locating buggy statements than the compared baselines.

Through ablation analysis on the impact of different features and modules of {\tool}, all designed features can contribute to {\tool}'s performance. Specifically, the proposed dual-learning can significantly improve the statement-level fault localization by up to 100\% in terms of EXAM score. The designed features of co-change relations among methods and statements can all have an impact on {\tool}. 
