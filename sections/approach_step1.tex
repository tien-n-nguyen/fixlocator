...
3) {\em \underline{Co-change relation}}: we add the
...
list of variables is \code{tree}, \code{BSPTree},
\code{Euclidean2D}, etc.
...
in~\cite{}, the relations among statements in an PDG are important in
FL, we integrate them into the statement-level feature graph. We use
Soot~\cite{soot} to build the PDG from source code. In
...
at line 4 has a control/data dependence with the one at line 5, which
connects to the ones at lines 7--8, and the ones at lines
10--11.

2) {\em \underline{Execution flow in execution trace}}: if two
...
changed together in a commit, thus, there is a co-change edge between them.
