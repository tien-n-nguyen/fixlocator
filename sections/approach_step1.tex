...
$M_1$. \underline{First}, for the method content, we remove special
...
$M_1$ represents the method \code{computeGeometricalProper\-ties} in
Figure~\ref{statement-level-feature-extraction}. For the method
content for $M_1$, the extracted sequence of sub-tokens is
\code{protected}, \code{void}, \code{compute}, \code{Geometrical},
\code{Properties}, etc. \underline{Second}, for the method's
structure, we use JDT package~\cite{JDT} to parse the source code to
build the AST of the method. \underline{Third}, we keep the most
...
be in the current version of the project. Two methods are similar when
they have similar sequences of the sub-tokens (represented by the
GloVe embeddings~\cite{glove2014}).  For $M_b$, we build its AST via
JDT and keep as an attribute for $M_1$.
...
(Figure~\ref{method-level-feature-extraction}). We encode as the edges
three types of relations. The \underline{first type} of relation is
the \underline{calling relation} in a \underline{stack trace} of a failed test case as we ran
it. Because the stack trace can be very long, from the failing/crash
point, we collect only part of the stack trace with $n$ levels of
depth from that point. From a previous
...
\underline{execution trace}. The calling relations among any two
...
calling relations (in green color) in some execution traces.  Finally,
we also add the \underline{co-change relations} among the methods into
the feature graph in which there is a co-change relation between two
methods that were changed together in the same commit. Note: we make a
co-change edge into two one-directional edges (as seen the one between
$M_5$ and $M_6$ in Figure~\ref{method-level-feature-extraction}).
