...
statement levels. GCN processes the attributes of the nodes and their
relations (edges) in feature graphs. Each GCN model has $n-1$ pairs of
a graph convolution layer (\code{Conv}) and a rectified linear unit
...
classifier on whether a node for a method or a statement is labeled as
faulty or non-faulty.

\noindent {\bf Dual Learning with Cross-stitch Units.} In a regular GCN
...
(\code{Conv}) of the \code{methFL} and \code{stmtFL} models.
