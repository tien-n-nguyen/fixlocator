...
{\em dual learning} task between them. First, the {\em method-level
  pairing} model (\code{MethFL}) aims to learn the methods that need
to be modified in the same fix. Second, the {\em statement-level
  pairing} model (\code{StmtFL}) aims to learn the co-fixing
...
joinly train \code{MethFL} and \code{StmtFL} with soft-sharing the
