...
fixing by developers, an FL tool needs point out all the buggy
statements to be fixed at the same time. Otherwise, (s)he must find
...
It is important for an FL tool to connect multiple buggy statements in
potentially different methods.

Despite their successes, the state-of-the-art fault localization (FL)
approaches {\em do not support the detection of multiple locations
  ...
  Fixing Locations} (CC Fixing Locations). All the FL models return a
ranked list of suspicious statements according to the corresponding
suspiciousness scores. In this example, the lines 3, 14, 19, 22, and
other lines ({\em e.g.}, 5, 13, 21 and 25) are executed in the same
passing or failing test cases, thus, are assigned the same scores by
spectrum-based and mutation-based FL approaches. A developer would not
be confused with that result on what lines need to be fixed together.
The ML-based FL approaches do not have any mechanism to detect CC
Fixing Locations.
...
confuse a developer. Moreover, another naive solution would be to
...
experiment, the inaccuracy of the first phase will have confounding
