...
program dependencies among statements. Our empirical results on real
datasets~show that {\tool} relatively improves over the
state-of-the-art statement-level FL baselines by locating {\bf
26.5\%--155.6\%} more CC fixing statements.
%from {\bf 26.5\% to 155.6\%}.
%
To evaluate its usefulness, we used it in combination with the
state-of-the-art APR tools -- DEAR and CURE.
%DEAR and CURE in which we replaced their FL modules with our tool. 
The result shows that {\tool}+DEAR (its original FL replaced by
{\tool}) and {\tool}+CURE improve relatively over the original DEAR
and Ochiai+CURE 10.5\% and 42.9\% in the numbers of fixed bugs.
\end{abstract}
