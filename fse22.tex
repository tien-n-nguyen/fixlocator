...
soft-sharing the models' parameters via cross-stitch units to
%
...
novel feature, which is the co-changed
%information among
...
baselines by locating more CC fixing statements from {\bf 26.5\% to
155.6\%}, and reduces the statements to be examined by {\bf
22\%--30\%}.
%
To show its usefulness, we used {\tool} to replace the original CC
fixing-location detection module of DEAR, an automated program repair
tool. The result shows that {\tool} helps DEAR relatively improve {\bf
XX}\% in terms of the numbers of bug fixes in comparison with that
