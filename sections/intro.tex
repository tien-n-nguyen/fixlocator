...
DeepFL~\cite{DeepFL}, DeepRL4FL \cite{icse21-fl}, and
DEAR~\cite{icse22} by 16.6\%, 16.9\%, 9.9\%, and {\bf x.x\%}
respectively, in terms of Hit-1 (i.e., the percentage of bugs in which
the predicted set overlaps with the oracle set {\em at least} one
faulty statement), and by 33.6\%, 40.3\%, 26.5\%, and {\bf xx.x\%} in
terms of Hit-2 (i.e., the number of overlapping statements $\geq$2),
43.9\%, 46.4\%, 28.1\%, and {\bf xx.x\%} in terms of Hit-3, respectively.
%, and 100\%, 155.6\%, 64.5\%, and {\bf xx.x\%} in terms of Hit-4,
%respectively.
{\tool} also improves over those baselines by {\bf xx.x\%}, {\bf
